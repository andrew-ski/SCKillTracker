...
$configPath = "$env:LOCALAPPDATA\SCKillTracker.cfg"
...
function Get-Directory()
{
    ...
    elseif(Test-Path -Path $configPath){
        $script:filepath = Get-Content $configPath
        $script:defaultdirectory = $True
        }
    else{
        ...
        if(Test-Path -Path $filepath){
            $filepath > "$env:LOCALAPPDATA\SCKillTracker.cfg"
            $script:defaultdirectory = $True
            }
        else{
...
#If config file exists, change filepath to content from config and set defaultdirectory to true
#Else if no config file exists, check if game.log exists in the default C drive directory, set default directory to true
#Else, ask user for file path and write a config file with their response, set file path and default directory
{
<
    $script:defaultdirectory = Test-Path -Path $filepath
	    if($defaultdirectory){
            ...
            $testDirectory = Test-Path -Path $filepath
            #add file path
            New-Item -Path "$($env:LOCALAPPDATA)\SCKillTracker.txt"
            if($testDirectory){
                $script:defaultdirectory = $True
            }
            else{
            ...
            if($charArray.Length -gt 21 -and $CharArray[3] -eq "Death>"){
                ...
                if($killed.Contains($killedId) -eq $false){
                  <# $spacingCheck = "$killer killed $killed"
                   ...
                   #>
                   $killLine = "$timestamp | $killer killed $killed | Weapon:$weapon | Damage: $damageType | Zone:$zone"
